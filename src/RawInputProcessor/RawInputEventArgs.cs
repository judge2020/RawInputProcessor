...
namespace RawInputProcessor
{
    public sealed class RawInputEventArgs : EventArgs
    {
        ...
        public int AdjustedVirtualKey { get; private set; }
        public bool Handled { get; set; }

        internal RawInputEventArgs(RawKeyboardDevice device, KeyPressState keyPressState, uint message, int adjustedVirtualKey,  int virtualKey)
        {
            Device = device;
            KeyPressState = keyPressState;
            Message = message;
            AdjustedVirtualKey = adjustedVirtualKey;
