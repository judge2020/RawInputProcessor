...
namespace RawInputProcessor
{
	public sealed class RawKeyboard : IDisposable
	{
		...
		public RawKeyboard(IntPtr hwnd, bool captureOnlyInForeground)
		{
			RawInputDevice[] array =
			{
				new RawInputDevice
				{
					...
					Flags =
						(captureOnlyInForeground ? RawInputDeviceFlags.NONE : RawInputDeviceFlags.INPUTSINK) |
		...
		{
			var notifyHandle = IntPtr.Zero;
			var broadcastDeviceInterface = default(BroadcastDeviceInterface);
			...
			var interfacePtr = Marshal.AllocHGlobal(Marshal.SizeOf(typeof(BroadcastDeviceInterface)));
			try
			{
		...
		public void EnumerateDevices()
		{
			lock (_lock)
			{
				...
				var pRawInputDeviceList = Marshal.AllocHGlobal((int) (size * devices));
				try
				{
					...
					while (index < devices)
					{
						var device = GetDevice(pRawInputDeviceList, size, index);
		...
		private static RawKeyboardDevice GetDevice(IntPtr pRawInputDeviceList, int dwSize, int index)
		{
			...
			var rawInputDeviceList =
				(RawInputDeviceList)
				Marshal.PtrToStructure(new IntPtr(pRawInputDeviceList.ToInt64() + dwSize * index), typeof(RawInputDeviceList));
			Win32Methods.GetRawInputDeviceInfo(rawInputDeviceList.hDevice, RawInputDeviceInfo.RIDI_DEVICENAME, IntPtr.Zero,
				ref size);
			if (size <= 0u)
				return null;
			var intPtr = Marshal.AllocHGlobal((int) size);
			try
			{
				Win32Methods.GetRawInputDeviceInfo(rawInputDeviceList.hDevice, RawInputDeviceInfo.RIDI_DEVICENAME, intPtr, ref size);
				var device = Marshal.PtrToStringAnsi(intPtr);
				if (rawInputDeviceList.dwType == DeviceType.RimTypekeyboard ||
				    rawInputDeviceList.dwType == DeviceType.RimTypeHid)
				{
					var deviceDescription = Win32Methods.GetDeviceDescription(device);
