...
namespace RawInputProcessor.Win32
{
	internal static class Win32Methods
	{
		...
		public static string GetDeviceDiagnostics()
		{
			...
			var devices = 0u;
			var listSize = Marshal.SizeOf(typeof(RawInputDeviceList));
			...
			try
			{
				...
				while (index < devices)
				{
					...
					var rawInputDeviceList =
						(RawInputDeviceList)
						Marshal.PtrToStructure(new IntPtr(deviceListPtr.ToInt64() + listSize * index), typeof(RawInputDeviceList));
		...
		public static string GetDeviceDescription(string device)
		{
			...
			var text = deviceKey.GetValue("DeviceDesc").ToString();
